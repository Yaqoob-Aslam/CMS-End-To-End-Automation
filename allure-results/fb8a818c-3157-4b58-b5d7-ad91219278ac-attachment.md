...
```
Error: locator.click: Error: strict mode violation: getByRole('option') resolved to 269 elements:
...
Call log:
  - waiting for getByRole('option')
    ...
    at TimetablePage.createTimetable (/home/yaqoob/CMS-End-to-End-Automation-with-Playwright/pages/TimetablePage.js:38:65)
...
- dialog "Add New Time Table":
  ...
  - textbox "Enter time table name": Timetable U5846
  - text: Scenario *
  - subscript
  - subscript
  - log: 269 results available.Use Up and Down to choose options, press Enter to select the currently focused option, press Escape to exit the menu, press Tab to select the option and exit the menu.
...
```ts
  ...
  13 |     this.startTimeInput = page.getByPlaceholder('Select start time');
  14 |     this.endTimeInput = page.getByPlaceholder('Select end time');
  ...
  21 |    
  22 |   async goto() {
  23 |     await this.menuTimetable.click();
  24 |     await expect(this.page).toHaveURL(/.*timetable/);
  25 |   }
  26 |
  27 |   async createTimetable(timetableName, scenarioName) {
  28 |     const now = new Date();
  29 |     const startTime = now.toTimeString().slice(0, 5); // e.g. "14:23"
  30 |     const endTime = new Date(now.getTime() + 5 * 60000).toTimeString().slice(0, 5);
  31 |
  32 |     await this.goto();
  33 |     await this.addTimetableBtn.click();
  34 |     await this.timetableNameInput.fill(timetableName);
  35 |
  36 |     // Select scenario
  37 |     await this.scenarioDropdown.click();
> 38 |     await this.page.getByRole('option', { name: scenarioName }).click();
     |                                                                 ^ Error: locator.click: Error: strict mode violation: getByRole('option') resolved to 269 elements:
  39 |
  40 |     // Set time range
  41 |     await this.startTimeInput.fill(startTime);
  42 |     await this.endTimeInput.fill(endTime);
  43 |
  44 |     // Save
  45 |     await this.addBtn.click();
  46 |     await this.doneBtn.click();
  47 |
  48 |      // ✅ Validation
  49 |     // await expect(this.page.getByText(timetableName)).toBeVisible();
  50 |   }
  51 | }
  52 |
```