...
```
Error: locator.waitFor: Error: strict mode violation: getByRole('option') resolved to 274 elements:
...
Call log:
  - waiting for getByRole('option') to be visible

    at TimetablePage.createTimetable (/home/yaqoob/CMS-End-to-End-Automation-with-Playwright/pages/TimetablePage.js:38:23)
...
- dialog "Add New Time Table":
  ...
  - textbox "Enter time table name": Timetable U72852
  - text: Scenario *
  - subscript
  - subscript
  - log: 274 results available.Use Up and Down to choose options, press Enter to select the currently focused option, press Escape to exit the menu, press Tab to select the option and exit the menu.
...
- listbox:
  ...
  - option "Scenario U68679"
  - option "Scenario U84310"
  - option "Scenario U72852"
```
...
```ts
  ...
  18 |   }
  19 |
  20 |   // Navigate to Timetable Page
  21 |   async goto() {
  22 |     await this.menuTimetable.click();
  23 |     // await expect(this.page).toHaveURL(/.*timetable/);
  24 |   }
  25 |
  26 |  async createTimetable(timetableName, scenarioName) {
  27 |   const now = new Date();
  28 |   const startTime = now.toTimeString().slice(0, 5);
  29 |   const endTime = new Date(now.getTime() + 5 * 60000).toTimeString().slice(0, 5);
  30 |
  31 |   await this.goto();
  32 |   await this.addTimetableBtn.click();
  33 |   await this.timetableNameInput.fill(timetableName);
  34 |
  35 |   // Select scenario safely
  36 |   await this.scenarioDropdown.click();
  37 |   const optionLocator = this.page.getByRole('option', { name: scenarioName, exact: true });
> 38 |   await optionLocator.waitFor({ state: 'visible', timeout: 5000 });
     |                       ^ Error: locator.waitFor: Error: strict mode violation: getByRole('option') resolved to 274 elements:
  39 |   await optionLocator.click();
  40 |
  41 |   // Set time range
  42 |   await this.startTimeInput.fill(startTime);
  43 |   await this.endTimeInput.fill(endTime);
  44 |
  45 |   // Save
  46 |   await this.addBtn.click();
  47 |   await this.doneBtn.click();
  48 |  }
  49 | }
  50 |
```